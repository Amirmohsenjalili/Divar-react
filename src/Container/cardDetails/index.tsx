...
//redux
import { useGetDetailsCardQuery } from "src/services/rtkQueryDetailsCard";

import { Params } from './cardDetails.types'

const CardDetails: React.FC = () => {
  const {id:token} = useParams<Params>()
  const { data, isLoading, error } = useGetDetailsCardQuery({token});
      ...
      <div className=" flex flex-col justify-center items-center w-auto mx-auto my-0 lg:block ">
        ...
        >
          ...
          {data?.analytics?.cat1 === "real-estate" ?
           ...
           data?.analytics?.cat1 === "craftsmen" ? 
           data?.sections[4]?.widgets[0]?.data?.parent_items[2]?.title :
           data?.sections[0]?.widgets[0]?.data?.parent_items[2]?.title}
          </span>
        </Link>
        <div className="flex flex-col items-center w-4/5 lg:w-full lg:flex lg:flex-row lg:items-start">
          <div className={style.right_section}>
            <div className=" flex flex-col items-center justify-center text-center lg:text-right lg:items-start ">
              ...
              {data?.analytics?.cat1 === "real-estate" ?
                data?.sections[1]?.widgets[0]?.data?.subtitle : 
                data?.analytics?.cat1 === "craftsmen" ? 
                null :
                data?.sections[1]?.widgets[0]?.data?.subtitle}
              </div>
              ...
              <div>
                ...
                <div>
                  <div className="min-w-0">
                    ...
                    {data?.analytics?.cat1 === "real-estate" ?
                      data?.sections[3]?.widgets[1]?.data?.text : 
                      data?.analytics?.cat1 === "craftsmen" ? 
                      data?.sections[2]?.widgets[1]?.data?.text :
                      data?.sections[2]?.widgets[1]?.data?.text}
                    </p>
                  </div>
                </div>
          ...
          <div className={`${style.left_section} mt-10 lg:mr-auto lg:mt-0 `}>
            <Slider cardImage=
              {data?.analytics?.cat1 === "real-estate" ?
              data?.sections[0]?.widgets[0]?.data?.items : 
              data?.analytics?.cat1 === "craftsmen" ? 
              data?.sections[0]?.widgets[0]?.data?.items :
              data?.sections[3]?.widgets[0]?.data?.items}
             />
